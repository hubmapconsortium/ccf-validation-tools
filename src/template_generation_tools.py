...
def generate_class_graph_template(ccf_tools_df :pd.DataFrame, log_dict: dict):
  ...
  if ccf_tools_df.empty:
    return (pd.DataFrame.from_records(records), pd.DataFrame.from_records(no_valid_records), error_log, ConjunctiveGraph(), valid_error_log, report_relationship, strict_log, unknown_terms,
  ...
  return (pd.DataFrame.from_records(records), pd.DataFrame.from_records(no_valid_records), error_log.sort_values('deltaIC', ascending=False), annotations, valid_error_log.sort_values('s'), report_relationship, strict_log.sort_values('s'), 
          unknown_terms, has_part_report.sort_values('s'), pd.DataFrame.from_records(records_ub_sub).drop_duplicates(), pd.DataFrame.from_records(records_cl_sub).drop_duplicates(), pd.DataFrame.from_records(image_report).sort_values('term'), sec_graph, log_dict)
