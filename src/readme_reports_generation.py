"""
The script defines a function `generate_template_readme` that takes a file
name and a table as input.
It generates a markdown template for the reports related to the ASCT+B
validation process.

The template includes sections for invalid terms, relationship reports, new CL
terms, new UBERON terms, and informative reports.
Each section provides a description of the report and its purpose.

The function uses the `MdUtils` class from the `mdutils` library to create
the markdown template.
It also imports other necessary libraries such as `argparse`, `json`,
`datetime`, `pandas`, and `tabulate`.

Note: The script assumes that there is a `download_resource.py` file in
the same directory, which contains the `get_sheet_gid` function.
"""
import argparse
import json
...
def generate_template_readme(file_name, table):
    """
    Generate a markdown template for the reports of the ASCT+B validation
    process.

    Args:
      file_name (str): The name of the output markdown file.
      table (str): The name of the table for which the reports are generated.
    ...
    template = MdUtils(
        file_name=file_name,
        title=f"ASCT+B Validation Reports for {table} ({date})"
    )

    template.new_header(
        level=1, title="Invalid terms", add_table_of_contents="y"
    )
    template.new_paragraph(
        text="""
            These are the reports related to issues in the terms found in the ASCT+B table.
            We validate only [CL](https://www.ebi.ac.uk/ols4/ontologies/cl), [UBERON](https://www.ebi.ac.uk/ols4/ontologies/uberon) and [PCL](https://www.ebi.ac.uk/ols4/ontologies/pcl) terms.
        """
    )

    template.new_header(
        level=2, title="Terms not found", add_table_of_contents="y"
    )
    template.new_paragraph(
        text="""
        This report provides a list of terms not found neither in UBERON
            nor in CL. Please remove these terms from the ASCT+B table -
            disconsider this message if a term was recently added to the ontology.
        """
    )
    template.new_line()
    template.new_line()
    ...
    template.new_header(
        level=2,
        title="Typos or punctuation mistakes",
        add_table_of_contents="y"
    )
    template.new_paragraph(
        text="""
            This report provides a general quality check of the terms used in
                the ASCT+B table.
            Typos, font case (upper case), punctuation mistakes in IDs: 
                two colons, spaces, underscore instead of a colon.
        """
    )
    template.new_line()
    template.new_line()
    ...
    template.new_paragraph(
        text="""
            This report provides a list of terms having different names/labels
            found in the ontology and related to the one found in the ASCT+B
                table. Make sure to add the term's name/label in the column AS/N/LABEL or CT/N/LABEL. If the SME wants to give another name/label, please use the column AS/N or CT/N. N is the number in the column.
            """
    )
    ...
    template.new_header(
        level=2,
        title="Blank ontology ID missing parent",
        add_table_of_contents="y"
    )
    ...
    template.new_header(
        level=2,
        title="Relationship AS-AS report",
        add_table_of_contents="y"
    )
    ...
    template.new_header(
        level=2,
        title="Relationship CT-CT report",
        add_table_of_contents="y"
    )
    ...
    template.new_header(
        level=2,
        title="Relationship CT-AS report",
        add_table_of_contents="y"
    )
    ...
    template.new_header(
        level=1,
        title="Informative reports (valid relationships)",
        add_table_of_contents="y"
    )
    ...
    template.new_header(
        level=2,
        title="Relationship AS has part CT",
        add_table_of_contents="y"
    )
...
def get_row_link(table, row):
    """
    Get the URL link for a specific row in the ASCT+B table.
    """
...
def compact_issues(items: list, element: str):
    """
    Compact a list of items based on a specific element.
    """
    compacted_items = {}
...
def list_rows_link(table: str, rows: list):
    """
    Generate a list of links for the rows in the ASCT+B table.
    """
    return (", ").join(
        list(map(lambda x: f"_[{x}]({get_row_link(table, x)})_", rows))
    )


def generate_invalid_terms_report(log_dict, table):
    """
    Generate a report for the invalid terms found in the ASCT+B table.
    """
    terms_report = {
        "no_found_id": "",
        "typos": "",
    ...
    def add_issue_to_report(issue_list, report_key, message_template):
        if issue_list:
            compacted_issues = compact_issues(
                issue_list,
                "id" if report_key == "typos" or report_key == "external" else "user_label"
            )
...
def add_base_iri(content):
    """
    Add the base IRI to the content.
    """
...
def generate_graph_template(file, table):
    """
    Generate a markdown template for the graph representation
    of the ASCT+B table.
    """
    date = datetime.today().strftime("%Y-%m-%d")

    template = MdUtils(
        file_name=file, title=f"ASCT+B Validation Reports for {table} ({date})"
    )
...
def add_row_n_term_link(report, table):
    """
    Add the row number and term link to the report.
    """
...
def split_report(report):
    """
    Split the relationship report into three parts.
    """
...
def tsv2md(report):
    """
    Convert the TSV report to a markdown table.
    """
...
def generate_relationship_md(table):
    """
    Generate the relationship reports for the ASCT+B table.
    """
...
if __name__ == "__main__":
    ...
    if args.mode == "readme":
        log = json.load(open(args.data, encoding="utf-8"))
        args.data = log
